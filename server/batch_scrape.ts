...
async function startMigration() {
    ...
    console.log(`🚀 Starting migration for ${allIds.length} records via HTTP API...`);

    // Fetch existing IDs from remote API
    console.log('🔍 Checking remote server for existing records...');
    const existingRes = await axios.get(`${API_BASE_URL}/technologies/ids`);
    const existingIds = new Set(existingRes.data);
    console.log(`⏭️ Found ${existingIds.size} records on server. Skipping already imported records.`);
                ...
                try {
                    ...
                    const payload = {
                        tech: {
                            ...
                            tech_category_id: details.category ? details.category.split(',')[0].trim() : 'General',
                            description: details.valueProposition || 'No description available.',
                            ip_status: details.patent && details.patent !== 'N/A' ? 'patented' : 'know-how',
                            patent_number: details.patent && details.patent !== 'N/A' ? details.patent : null,
                            trl_level: trlLevel
                        },
                        stakeholder: {
                            stakeholder_id: stakeholderId,
                            name: details.labName,
                    ...
                    console.log(`✅ Imported ${id}: ${details.title}`);
                } catch (apiErr: any) {
                    console.error(`❌ API Error for ${id}:`, apiErr.response?.data?.error || apiErr.message);
                }
            } else {
                console.error(`❌ Failed to scrape ${id} after retries.`);
            }

            // Randomized delay between individual records (2-5 seconds)
            const delay = 2000 + Math.random() * 3000;
            await new Promise(r => setTimeout(r, delay));
        }

        if (i + CHUNK_SIZE < remainingIds.length) {
            console.log(`\n😴 Chunk ${chunkNum} complete. Cooling down for 5 minutes...`);
            await new Promise(r => setTimeout(r, 5 * 60 * 1000));
            // Re-fetch session after long break
            sessionCookie = '';
        }
    }
